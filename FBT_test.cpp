...
TEST(FullBinaryTreeTest, SaveLoadBinary) {
    ...
    ASSERT_TRUE(check.is_open()); //assert_true - проверяет что условие true
    check.close();
...
TEST(FullBinaryTreeTest, SaveLoadTextFile) {
    FullBinaryTree tree;
    tree.TINSERT(5);
    tree.TINSERT(3);
    tree.TINSERT(7);
    ...
    tree.saveToTextFile("fulltree_test.txt");

    ifstream check("fulltree_test.txt");
    ASSERT_TRUE(check.is_open());
    check.close();

    
    tree.loadFromTextFile("fulltree_test.txt");

    ASSERT_TRUE(tree.ISMEMBER(5));
    ASSERT_TRUE(tree.ISMEMBER(3));
    ASSERT_TRUE(tree.ISMEMBER(7));
    ASSERT_TRUE(tree.ISMEMBER(2));
    ASSERT_TRUE(tree.ISMEMBER(4));

    remove("fulltree_test.txt");
}
