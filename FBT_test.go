...
// Тест для бинарной сериализации
...
// текст
func TestFullBinaryTreeSaveLoadTextFile(t *testing.T) {
	tree := NewFullBinaryTree()
	tree.TINSERT(5)
	tree.TINSERT(3)
	tree.TINSERT(7)
	...
	err := tree.SaveToTextFile("fulltree_test.txt")
	if err != nil {
		t.Fatal(err)
	}

	if _, err := os.Stat("fulltree_test.txt"); os.IsNotExist(err) {
		t.Fatal("File should have been created")
	}

	tree2 := NewFullBinaryTree()
	err = tree2.LoadFromTextFile("fulltree_test.txt")
	if err != nil {
		t.Fatal(err)
	}
